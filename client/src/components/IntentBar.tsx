...
export function IntentBar() {
  ...
  return <section className="panel panel-pad" aria-label="Set intent">
    ...
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key={draft.mode} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={springs.spring} style={{ overflow: 'hidden', marginBottom: 16 }}>
        ...
        {draft.mode === 'vibe' && <textarea style={{ width: '100%', minHeight: 70, resize: 'vertical' }} value={draft.vibeText} onChange={(e) => updateDraft({ vibeText: e.target.value })} placeholder="golden hour brazil beach party" />}
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'end', gap: 10 }}>
      <label><span className="label">set size</span><div><input className="mono" style={{ width: 68 }} type="number" min="1" value={constraints.setSize.value} onChange={(e) => updateConstraints({ setSize: { ...constraints.setSize, value: Number(e.target.value) || 1 } })} /><select value={constraints.setSize.type} onChange={(e) => updateConstraints({ setSize: { ...constraints.setSize, type: e.target.value as 'count' | 'minutes' } })}><option value="count">tracks</option><option value="minutes">minutes</option></select></div></label>
      <label><span className="label">energy</span><select value={constraints.energyBehavior} onChange={(e) => updateConstraints({ energyBehavior: e.target.value as typeof constraints.energyBehavior })}><option value="flat">flat</option><option value="rise">rise</option><option value="rise-peak-cooldown">rise / peak / cooldown</option><option value="custom">custom</option></select></label>
      <label style={{ minWidth: 144 }}><span className="label">harmony · {strictnessLabel(constraints.harmonicStrictness)}</span><input style={{ display: 'block', width: '100%' }} type="range" min="0" max="1" step=".05" value={constraints.harmonicStrictness} onChange={(e) => updateConstraints({ harmonicStrictness: Number(e.target.value) })} /></label>
      <label><span className="label">BPM max Δ</span><input className="mono" style={{ width: 70 }} type="number" min="1" value={constraints.bpmPolicy.maxDeltaPercent} onChange={(e) => updateConstraints({ bpmPolicy: { ...constraints.bpmPolicy, maxDeltaPercent: Number(e.target.value) || 1 } })} /></label>
      <button className="btn primary" disabled={!needed || stage !== 'idle'} onClick={() => void generate()}>Architect the set</button>
    </div>
