...
export class ClaudeCliAdapter implements BrainAdapter {
  ...
  async complete(prompt:string, opts:{timeoutMs?:number}={}) { try { const r=await execa(this.path,['-p',prompt,'--output-format','json'],{timeout:opts.timeoutMs,reject:true,preferLocal:false}); try { return JSON.parse(r.stdout).result ?? r.stdout; } catch { return r.stdout; } } catch (error) { if(process.platform==='win32' && this.path==='claude') { const r=await execa('cmd',['/c','claude.cmd','-p',prompt,'--output-format','json'],{timeout:opts.timeoutMs}); try{return JSON.parse(r.stdout).result??r.stdout;}catch{return r.stdout;} } throw error; } }
