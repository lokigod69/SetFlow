...
const shape = '{"pool":[{"artist":"","title":"","mix":"","estBpm":120,"estKey":"8A","estEnergy":5,"moodTags":[],"slot":"","why":""}],"options":[{"id":"A","label":"","rationale":"","order":[0],"transitions":[{"blend":"long-blend","note":""}]}],"arcs":{"A":[5]}}';
...
export function buildPrompt(request:BrainRequest, settings:Settings): string {
  ...
  return `You are a veteran DJ and selector with impeccable taste. Respond with ONLY a valid JSON object, no prose, no markdown fences. ${apply(template,{context,extra})} ${context}${extra} JSON shape example: ${request.kind==='replace' ? '{"replacements":[{"replacesIndex":0,"track":'+shape.slice(8,-1)+'}]}' : request.kind==='alternatives' ? '{"alternatives":['+shape.slice(8,-1)+']}' : shape}`;
}
